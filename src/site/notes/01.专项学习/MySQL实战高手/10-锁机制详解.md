---
{"dg-publish":true,"permalink":"/01.专项学习/MySQL实战高手/10-锁机制详解/","dg-note-properties":{"时间":"2026-03-22","sr-due":"2026-03-29","sr-interval":3,"sr-ease":250}}
---

#mysql #数据库 #锁 #并发 #review 
...
| **按粒度** | 全局锁 | 整个库只读，用于备份 |
| | 表锁 | 锁整张表，粒度粗 |
| | 行锁 | 锁单行，InnoDB 特有，粒度最细 |
| **按模式** | 共享锁（S） | 读锁，多个事务可同时持有 |
| | 排他锁（X） | 写锁，独占，与其他所有锁互斥 |
| | 意向共享锁（IS） | 表级锁，表示"表里有行加了 S 锁" |
| | 意向排他锁（IX） | 表级锁，表示"表里有行加了 X 锁" |
| **按算法** | 记录锁 | 锁单条索引记录 |
| | 间隙锁 | 锁索引记录之间的间隙，防止幻读 |
| | 临键锁 | 记录锁 + 间隙锁，InnoDB 默认方式 |
...
实际业务很少直接 `LOCK TABLES` 加表锁，但 **ALTER TABLE 等 DDL 操作**会自动加 MDL 写锁，这时候要检查表里有没有行正在被操作。没有意向锁就得逐行扫描，有了意向锁看一眼表级标志就行。

不过平时不用关心它，MySQL 自动管理的。
