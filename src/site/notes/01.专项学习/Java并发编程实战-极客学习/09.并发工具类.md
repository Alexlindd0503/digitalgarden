---
{"dg-publish":true,"permalink":"/01.专项学习/Java并发编程实战-极客学习/09.并发工具类/"}
---
...
`Lock` 解决互斥，`Condition` 解决同步（是 [[01.专项学习/Java并发编程实战-极客学习/07.管程Monitor\|管程]] 条件变量的显式实现，支持多个等待队列，比 `wait/notify` 更灵活）。底层原理见 [[66.归档发布/04.并发/AQS抽象队列同步器原理\|AQS抽象队列同步器原理]]。
...
**可见性保障**：Lock 内部有 `volatile` 的 `state` 变量，结合 [[01.专项学习/Java并发编程实战-极客学习/04.Java内存模型JMM#^9428f5\|happens-before 规则]] 保证可见性。

**可重入锁**：同一线程可以重复获取同一把锁（`ReentrantLock`）。

**公平锁 vs 非公平锁**：公平锁按等待时间唤醒，线程切换频繁，性能低；默认用非公平锁。

> Lock+Condition 可实现异步转同步（如 RPC 调用等待结果），这也是 [[01.专项学习/Java并发编程实战-极客学习/10.并发设计模式#Guarded Suspension 模式（保护性暂停）\|Guarded Suspension 模式]] 的实现基础。
...
| 容器 | 特点 |
|------|------|
| `CopyOnWriteArrayList` | 写时复制，适合读多写少，可容忍短暂不一致，原理见 [[01.专项学习/Java并发编程实战-极客学习/10.并发设计模式#Copy-on-Write 模式（写时复制）\|CoW 模式]] |
